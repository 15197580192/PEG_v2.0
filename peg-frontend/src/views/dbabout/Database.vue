...
export default {
  ...
  methods: {
    // 获取数据库
    getDatabaseList() {
      this.$http.get('/GetDatabaseName?user_id=' + this.system_user).then(
        (resp) => {
    ...
    Tomain() {
      ...
      let data = JSON.stringify({
        db_name: this.current_db.database_name,  //数据库名
        database_creater_id: this.system_user, //创建者ID 
    ...
    getdbInfo() {
      ...
      var data = JSON.stringify({
        db_name: db_name,
        database_creater_id: this.system_user,
        dbuser_id: "root"
      })
